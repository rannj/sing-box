...
func (h *httpHandler) recentConnections(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil || offset < 0 {
		writeError(writer, http.StatusBadRequest, "invalid_query_parameter", "offset must be zero or greater", "offset", "")
		return
	}
	limit, err := queryInteger(request, "limit", 100)
	...
	result, err := h.manager.recentConnectionPage(request.URL.Query().Get("cursor"), offset, limit, window)
	if err != nil {
		writeError(writer, http.StatusBadRequest, "invalid_query_parameter", err.Error(), "cursor", "")
		return
	}
