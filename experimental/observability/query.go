...
func paginateConnections(connections []Connection, cursorValue string, offset int, limit int, closed bool) (ConnectionPage, error) {
	if limit <= 0 {
		limit = 100
	}
	start := min(offset, len(connections))
	if cursorValue != "" {
