...
if [[ $family == ipv4 ]]; then
  ...
  ping_family=(-6)
  address_flags=(nodad)
  local_ipv6_mode=always
  shared_ipv6_mode=always
fi
server_port=
redirect_port=15001
tproxy_port=15002
...
start_sing_box() {
  ...
  case "$variant" in
    ebpf-local|ebpf-profile-local)
      ...
      inbound=$(jq -n --arg cgroup "$cgroup_path" --arg ipv6Mode "$local_ipv6_mode" '{
        type: "ebpf",
        tag: "benchmark-in",
        mode: "local",
        network: ["tcp", "udp"],
    ...
    ebpf-shared|ebpf-shared-leak-check|ebpf-profile-shared)
      ...
      inbound=$(jq -n --arg interface "$router_app_interface" --arg ipv6Mode "$shared_ipv6_mode" '{
        type: "ebpf",
        tag: "benchmark-in",
        mode: "shared",
        network: ["tcp", "udp"],
