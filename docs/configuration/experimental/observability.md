...
are rejected. A larger value increases memory usage and response time.
...
scrape_configs:
  - job_name: sing-box
    ...
    bearer_token: replace-with-a-long-secret
    static_configs:
...
Grafana can use Prometheus for trends and the `/connections/recent` JSON API
through an Infinity data source for a live request table.
