...
func normalizeCgroupMapCapacity(configured option.EBPFStateCapacity) (ECommon.CgroupMapCapacity, error) {
	capacity := ECommon.DefaultCgroupMapCapacity()
	if configured == 0 {
		return capacity, nil
	}
	value := uint32(configured)
	if value > ECommon.MaxConfigurableMapCapacity {
...
func normalizeSharedNetworkMapCapacity(configured option.EBPFStateCapacity) (ECommon.SharedNetworkMapCapacities, error) {
	...
	capacity.Proxy = value
	capacity.Bypass = value
	capacity.Fragment = value
	return capacity, nil
}
