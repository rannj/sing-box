...
type sharedTCManager struct {
	...
	attachmentRepairs atomic.Uint64
	attachmentAdds    atomic.Uint64
	attachmentRemoves atomic.Uint64
}
...
func (m *sharedTCManager) Start() error {
	...
	started := time.Now()
	err := m.reconcile()
	m.observeReconcile(started, err)
	if err != nil {
...
func (m *sharedTCManager) observeReconcile(started time.Time, err error) {
	if m.debug != nil {
		m.debug.observe(ebpfDebugTaskSharedAttachmentReconcile, time.Since(started), err)
	}
}
...
func (m *sharedTCManager) reconcile() (err error) {
	...
	for interfaceName, attachment := range m.attachments {
		...
		m.attachmentRemoves.Add(1)
		m.logger.Debug("eBPF shared-network detached from ", interfaceName)
	}
...
type sharedTCLifecycleSnapshot struct {
	ReconcileFailures uint64 `json:"reconcile_failures"`
	AttachmentRepairs uint64 `json:"attachment_repairs"`
	AttachmentAdds    uint64 `json:"attachment_adds"`
	AttachmentRemoves uint64 `json:"attachment_removes"`
}

type sharedTCAttachmentRuntimeStatus struct {
	Interface        string                                 `json:"interface"`
	InterfaceIndex   int                                    `json:"interface_index,omitempty"`
	Mode             string                                 `json:"mode"`
	Healthy          bool                                   `json:"healthy"`
	IngressProgramID int                                    `json:"ingress_program_id,omitempty"`
	EgressProgramID  int                                    `json:"egress_program_id,omitempty"`
	TCX              *ECommon.SharedNetworkTCXRuntimeStatus `json:"tcx,omitempty"`
	Error            string                                 `json:"error,omitempty"`
}

type sharedTCRuntimeStatus struct {
	Enabled     bool                              `json:"enabled"`
	Mode        string                            `json:"mode"`
	Lifecycle   sharedTCLifecycleSnapshot         `json:"lifecycle"`
	Attachments []sharedTCAttachmentRuntimeStatus `json:"attachments"`
}

func (m *sharedTCManager) lifecycleSnapshot() sharedTCLifecycleSnapshot {
	if m == nil {
		return sharedTCLifecycleSnapshot{}
	}
	return sharedTCLifecycleSnapshot{
		ReconcileFailures: m.reconcileFailures.Load(),
		AttachmentRepairs: m.attachmentRepairs.Load(),
		AttachmentAdds:    m.attachmentAdds.Load(),
		AttachmentRemoves: m.attachmentRemoves.Load(),
	}
}

func (m *sharedTCManager) runtimeStatus() sharedTCRuntimeStatus {
	if m == nil {
		return sharedTCRuntimeStatus{}
	}
	m.access.Lock()
	defer m.access.Unlock()
	status := sharedTCRuntimeStatus{
		Enabled:   m.enabled,
		Lifecycle: m.lifecycleSnapshot(),
	}
	tcxCount := 0
	for _, interfaceName := range m.interfaces {
		attachment, loaded := m.attachments[interfaceName]
		if !loaded {
			status.Attachments = append(status.Attachments, sharedTCAttachmentRuntimeStatus{
				Interface: interfaceName,
				Mode:      "none",
			})
			continue
		}
		attachmentStatus := sharedTCAttachmentRuntimeStatus{
			Interface:      interfaceName,
			InterfaceIndex: attachment.interfaceIndex,
		}
		if attachment.tcx != nil {
			tcxCount++
			attachmentStatus.Mode = "tcx"
			tcxStatus := attachment.tcx.RuntimeStatus(attachment.interfaceIndex)
			attachmentStatus.TCX = &tcxStatus
			attachmentStatus.Healthy = tcxStatus.Healthy
			attachmentStatus.IngressProgramID = int(tcxStatus.Ingress.ProgramID)
			attachmentStatus.EgressProgramID = int(tcxStatus.Egress.ProgramID)
		} else {
			attachmentStatus.Mode = "clsact"
			attachmentStatus.IngressProgramID = sharedTCFilterProgramID(attachment.ingress)
			attachmentStatus.EgressProgramID = sharedTCFilterProgramID(attachment.egress)
			attachmentStatus.Healthy, attachmentStatus.Error = sharedTCFiltersHealthy(attachment)
		}
		status.Attachments = append(status.Attachments, attachmentStatus)
	}
	if len(m.attachments) == 0 {
		status.Mode = "waiting"
	} else if tcxCount == len(m.attachments) {
		status.Mode = "tcx"
	} else if tcxCount == 0 {
		status.Mode = "clsact"
	} else {
...
func sharedTCFiltersHealthy(attachment *sharedTCAttachment) (bool, string) {
	link, err := netlink.LinkByIndex(attachment.interfaceIndex)
	if err != nil {
	...
	for _, expected := range []struct {
		...
		{attachment.ingress, netlink.HANDLE_MIN_INGRESS, sharedIngressFilterHandle, "sb_share_in"},
		{attachment.egress, netlink.HANDLE_MIN_EGRESS, sharedEgressFilterHandle, "sb_share_out"},
	} {
		if expected.filter == nil {
			return false, "filter is unavailable"
		}
		filters, listErr := netlink.FilterList(link, expected.parent)
		if listErr != nil {
		...
		for _, filter := range filters {
			...
			if isBPF && sharedTCFilterMatches(
				...
				expected.parent,
				netlink.MakeHandle(0, expected.handle),
		...
		if !found {
			return false, "filter is not attached"
		}
	}
	return true, ""
}
