...
func TestSharedNetworkDataPathIntegration(t *testing.T) {
	...
	if !runtimeStatus.Enabled || len(runtimeStatus.Attachments) != 2 ||
		!runtimeStatus.Attachments[1].Healthy ||
		runtimeStatus.Attachments[1].IngressProgramID == 0 ||
		runtimeStatus.Attachments[1].EgressProgramID == 0 {
		t.Fatalf("unexpected shared-network attachment runtime status: %+v", runtimeStatus)
	}
	backendStatus := backend.RuntimeStatus()
	if len(backendStatus.Maps) == 0 || len(backendStatus.Programs) != 2 {
		t.Fatalf("incomplete shared-network backend runtime status: %+v", backendStatus)
	}
	t.Cleanup(func() { _ = manager.closeAttachments() })

	attachment := manager.attachments[hostLink]
	expectedRepairs := uint64(1)
	...
	if snapshot := manager.lifecycleSnapshot(); snapshot.AttachmentRepairs < expectedRepairs {
		t.Fatalf("shared-network lifecycle repairs were not recorded: %+v", snapshot)
	}
	...
	go func() {
		...
		client := conn.RemoteAddr().(*net.TCPAddr).AddrPort()
		tokenDestination := conn.LocalAddr().(*net.TCPAddr).AddrPort()
		original, lookupErr := backend.LookupOriginal(ECommon.ProtocolTCP, client, tokenDestination)
		if lookupErr != nil {
			tcpResult <- lookupErr
			return
		}
