...
const (
	...
	integrationStressStatsEnv               = "SING_BOX_EBPF_STRESS_STATS"
...
func TestCgroupBackendTrafficStressIntegration(t *testing.T) {
	...
	if os.Getenv(integrationStressStatsEnv) == "1" {
		releaseStats, statsErr := AcquireProgramRuntimeStats()
		if statsErr != nil {
			t.Logf("kernel BPF runtime statistics unavailable: %v", statsErr)
		} else if releaseStats != nil {
			t.Cleanup(func() {
				if releaseErr := releaseStats(); releaseErr != nil {
					t.Errorf("release kernel BPF runtime statistics: %v", releaseErr)
				}
			})
		}
	}
	...
	redirectUsage, err := backend.RedirectMapUsage(ProtocolUDP)
	if err != nil {
		t.Fatal(err)
	}
	if backend.UsesSocketRelease() && redirectUsage.Entries != 0 {
		t.Fatalf("connected UDP redirects survived socket release: %+v", redirectUsage)
	}
	for _, program := range backend.RuntimeStatus().Programs {
		if program.StatsKnown && program.RunCount > 0 {
			t.Logf(
				"BPF program: name=%s section=%s runs=%d runtime=%s average=%d ns/run recursion_misses=%d",
				program.Name,
				program.Section,
				program.RunCount,
				time.Duration(program.RuntimeNanos),
				program.AverageNanos,
				program.RecursionMisses,
			)
		}
	}
	t.Logf(
...
func serveStressUDP(
	...
	for index := 0; index < count; index++ {
		...
		if err != nil {
			reservationFailures, reservationErr := backend.RedirectReservationFailures(ProtocolUDP)
			status := backend.RuntimeStatus()
			t.Fatalf(
				"read stress UDP packet %d: %v; redirect_reservation_failures=%d (%v); maps=%+v",
				index,
				err,
				reservationFailures,
				reservationErr,
